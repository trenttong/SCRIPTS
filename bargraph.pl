...
# =<bool_param>
#
# The script takes in a single file that specifies the data to graph and control 
# parameters for customizing the graph. The parameters must precede the data in the file. 
# Comments can be included in a graph file following the # character.
#
# The script's output, by default, is encapsulated postscript (.eps), which is sent to stdout.
# Simply redirect it to the desired output file:
#
# bargraph.pl mygraphfile > mygraphfile.eps
#
# The script first produces data to send to gnuplot, which can be seen by specifying -gnuplot. 
# Next, the script takes the resulting fig output from gnuplot and post-processes it to fill in the bars.
# The final fig data can also be selected via -fig. This data is then sent to fig2dev to produce a final figure.
#
# I keep my data in .perf files and have my Makefile generate .eps for latex and .png for slides or web pages. 
# See converting to non-vector formats for notes on avoiding aliasing and other problems when creating images, 
# and for some Makefile rules. My script magnifies 2x when converting to png to help avoid these problems, 
# but for most uses that's not enough and you should follow my suggestions rather than using -png. My default
# for -png produces a transparent background; the -non-transparent option disables that feature. 
# 
# For more information on the script. Please refer to http://www.burningcutlery.com/derek/bargraph/
#
# Have Fun ...
#
###########################################################################
